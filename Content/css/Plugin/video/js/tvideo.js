...
$(document).ready(function(){
    ...
    $('#tvplaylist > ul > li').click(function(){
        ...
        debugger;
        var path = src.split(".");
        if (path[path.length - 1].toLowerCase() == "pdf" ) {

            return true;
        }
        if (path[path.length - 1].toLowerCase() == "ppt" ) {

            return true;
        }
        if (path[path.length - 1].toLowerCase() == "doc" || path[path.length - 1].toLowerCase() == "docx") {

            return true;
        }
        else {
            var newClip = { 'url': src, 'autoplay': true };

            $f("player").play([{ url: src }]);
